...
import com.bond.android_ipc.BuildConfig;
...
public class FileAdapter {
  ...
  public static Uri create_export_Uri(String file_name, Context  context) {
    StringBuilder sb  = new StringBuilder(256);
    sb.append(context.getFilesDir().getPath())
        .append(File.separator)
        .append("specnet")
      ...
      try {
        ...
        if (null != in)  {
          ...
          sb.append(context.getFilesDir().getPath())
              .append(File.separator)
              .append("specnet");
          FileAdapter.saveFileI(sb.toString(), file_name, in);
        }
      } catch (Exception e) {
        Log.e(TAG, "create_export_Uri(): ", e);
      }
    }
  ...
  public static byte[] readFileBytes(String file, Context context)  {
    byte[]  re  =  readFileB(file);
    if (0 == re.length)  {
      StringBuilder sb  = new StringBuilder(256);
      sb.append(context.getFilesDir().getPath())
          .append("/specnet/").append(file);
      re  =  readFileB(sb.toString());
      if (0 == re.length)  {
        re  =  loadAssetBytes(context,  file);
      }
    }
    return re;
  }
...
//          }
  ...
  public static byte[] readBytesFromURI(Uri uri, Context context) {
    ByteArrayOutputStream fbuf = new ByteArrayOutputStream();
    try {
      ...
      Log.e(TAG,"readBytesFromURI err:",e);
    }
    return fbuf.toByteArray();
  }

  public static byte[] loadAssetBytes(Context context, String pathRelative)  {
    ByteArrayOutputStream fbuf = new ByteArrayOutputStream();
    try {
      InputStream in = context.getAssets().open(pathRelative);
      byte[] buf = new byte[2048];
  ...
  private static byte[] readFileB(String  file_path)  {
    File f = new File(file_path);
    if (!f.exists()) {  return new byte[0];  }
    long llen  =  f.length();
    int  ilen  =  (int) llen;
    if (llen  !=  ilen  ||  ilen  <  1) {  return new byte[0];  }
    ByteArrayOutputStream fbuf  =  new ByteArrayOutputStream(ilen);
    try {
      InputStream in  =  new FileInputStream(f.getAbsolutePath());
      byte[] buf = new byte[2048];
